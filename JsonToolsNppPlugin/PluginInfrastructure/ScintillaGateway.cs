...
namespace Kbg.NppPluginNET.PluginInfrastructure
{
    ...
    public class ScintillaGateway
    {
        ...
        private const int MARK_HIDELINESBEGIN = 19;
        private const int MARK_HIDELINESEND = 18;
        private const int MARK_HIDELINES_BEGIN_SHIFT = 1 << MARK_HIDELINESBEGIN;
        private const int MARK_HIDELINES_END_SHIFT = 1 << MARK_HIDELINESEND;

        /// <summary>
        /// * If line is not hidden (using the View->Hide Lines command from the Notepad++ main menu),
        ///     return false and set hideStart = -1 and hideEnd = -1
        /// * Otherwise, return true and set hideStart and hideEnd to the 0-indexed line numbers
        ///     of the start and end of the hidden line range that includes line.
        /// </summary>
        /// <param name="line"></param>
        /// <param name="hideStart"></param>
        /// <param name="hideEnd"></param>
        /// <returns></returns>
        public bool GetStartAndEndOfHiddenLines(int line, out int hideStart, out int hideEnd)
        {
            hideStart = -1;
            hideEnd = -1;
            if (GetLineVisible(line))
                return false;
            for (hideStart = line; hideStart >= 0; hideStart--)
            {
                if ((MarkerGet(hideStart) & MARK_HIDELINES_BEGIN_SHIFT) != 0)
                    break;
            }
            int lineCount = GetLineCount();
            for (hideEnd = line; hideEnd < lineCount; hideEnd++)
            {
                if ((MarkerGet(hideEnd) & MARK_HIDELINES_END_SHIFT) != 0)
                    break;
            }
            return true;
        }
        ...
        /// <summary>
        /// if line is in a range of lines hidden using the "View->Hide Lines" Notepad++ main menu command,
        ///     unhide all the lines in that range.
        /// </summary>
        /// <param name="line"></param>
        public void UnhideEnclosingHiddenLineRange(int line)
        {
            if (GetStartAndEndOfHiddenLines(line, out int hideStart, out int hideEnd))
            {
                ShowLines(hideStart, hideEnd);
                MarkerDelete(hideStart, MARK_HIDELINESBEGIN);
                MarkerDelete(hideEnd, MARK_HIDELINESEND);
            }
        }
    }
}
